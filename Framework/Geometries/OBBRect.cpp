...
OBBRect::OBBRect(Vector3 position, Vector3 size, float rotation)
	: position(position), size(size), rotation(rotation)
{
...

	{
		...
		vertices[0].position = D3DXVECTOR3(0.0f, 0.0f, 0.0f); // left bottom

		vertices[0].color = Color(1, 0, 0, 1);



		vertices[1].position = D3DXVECTOR3(0.0f, 1.0f, 0.0f); // left top

		vertices[1].color = Color(1, 0, 0, 1);



		vertices[2].position = D3DXVECTOR3(1.0f, 0.0f, 0.0f); // right bottom

		vertices[2].color = Color(1, 0, 0, 1);



		vertices[3].position = D3DXVECTOR3(1.0f, 1.0f, 0.0f); // right top
...
string OBBRect::MakeLabel(string preFix)
{
	string label = preFix + "##" + to_string((uint)this);
	return label;
}

void OBBRect::UpdateWorld()
{
